...
impl SemanticContext {
    ...
    pub(crate) fn evaluate<'a, T: Parser<'a>>(&self, parser: &mut T, outer_context: &(dyn ParserRuleContext<'a, TF=T::TF> + 'a)) -> bool {
        match self {
            ...
            SemanticContext::AND(ops) =>
                ops.iter().all(|sem| sem.evaluate(parser, outer_context)),
            SemanticContext::OR(ops) =>
                ops.iter().any(|sem| sem.evaluate(parser, outer_context)),
        }
    }
    pub(crate) fn eval_precedence<'a, 'b, T: Parser<'b>>(&'a self, parser: &T, outer_context: &(dyn ParserRuleContext<'b, TF=T::TF> + 'b)) -> Option<Cow<'a, SemanticContext>> {
        match self {
    ...
    pub fn new_or(a: &SemanticContext, b: &SemanticContext) -> SemanticContext {
        ...
        if let SemanticContext::OR(ops) = b {
            ops.iter().for_each(|it| { operands.insert(it.clone()); });
        } else {
            operands.insert(b.clone());
        }
    ...
    pub fn and(a: Option<impl Borrow<SemanticContext>>, b: Option<impl Borrow<SemanticContext>>) -> SemanticContext {
        match (a, b) {
            ...
            (Some(a), Some(b)) => {
                ...
                if *a == Self::NONE { return b.clone(); }
                if *b == Self::NONE { return a.clone(); }
    ...
    pub fn or(a: Option<impl Borrow<SemanticContext>>, b: Option<impl Borrow<SemanticContext>>) -> SemanticContext {
        match (a, b) {
            (None, None) => Self::NONE,
            (None, Some(b)) => b.borrow().clone(),
            (Some(a), None) => a.borrow().clone(),
...
fn filter_precedence_predicate(collection: &mut HashSet<SemanticContext>) -> Vec<SemanticContext> {
    let mut result = vec![];
    collection.retain(|it| if let SemanticContext::Precedence(_) = it {
