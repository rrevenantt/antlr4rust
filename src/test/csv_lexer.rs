...
const channelNames: [&'static str; 0 + 2] = ["DEFAULT_TOKEN_CHANNEL", "HIDDEN"];

const modeNames: [&'static str; 1] = ["DEFAULT_MODE"];

const ruleNames: [&'static str; 5] = ["T__0", "T__1", "T__2", "TEXT", "STRING"];
lazy_static! {
...
pub struct CSVLexer<'b> {
    pub base: BaseLexer<'b>,
...
impl<'b> Recognizer for CSVLexer<'b> {
    ...
    //	fn get_serialized_atn(&self) -> &'static str { _serializedATN }

    //pub fn get_vocabulary() -> &Vocabulary {&VOCABULARY }

    //	fn get_channel_names(&self) -> &'static [&'static str] {&channelNames}

    //	fn get_mode_names(&self) -> &'static [&'static str] {&modeNames}
}
...
impl<'b> CSVLexer<'b> {
    pub fn new(input: Box<dyn CharStream>) -> CSVLexer<'b> {
        CSVLexer {
